...
class Router
{
    ...
    foreach($GLOBALS['auth_groups'] as $auth_group){
      if(in_array($auth_group['auth_ref'], $route['auth']) && $auth_group['condition']){
        ...
        switch($route['type']){
          ...
          case "view":
            $filename = "Views/".$route['filename'];
            $MIME = mime_content_type($filename);
            switch($MIME){
              case 'text/plain':
                if(array_key_exists(".".pathinfo($route['filename'])['extension'], self::$mime_types)){
                  $MIME = self::$mime_types[".".pathinfo($route['filename'])['extension']];
                } else {
                  $MIME = "text/plain";
                }
            }
  ...
  /**
   * If it is an api request, it loops through api routes to check if the route is exactly equal to the current URI, if so, if the route type is api, it checks if the request method matches the method defined in the route or if its not an options request (for preflight requests). If the request method does not match the request method, http header status 405 is returned and the operation is exited. If the request method matches the route's method, the route info is returned (for both view and api request when the route is exactly the same as the request URI). If the request URI is not exactly the same as the route, the route is then tested if it is a regex pattern by checking for delimiters; if so, the request URI is then tested against the regex pattern in the route value, if there is a match, and the route type is view, the filename value is changed by replacing the closing captures from the request URI into the filename closing captures; irregardless of what route type it is, the closing captures from the request URI are intiated into the GET supervariable; and the route values are returned. If the loop is completed without returning route values, an http response code 404 is returned and the operation is exited.
   *
  ...
  public static function checkRouteExists()
  {
    foreach($GLOBALS['routes'] as $route){
      ...
      if(preg_match('/^(\/|\#|\~|\%|\+)(.*)(\1)$/', $route['route'])){
        if(preg_match($route['route'], $_SERVER['REQUEST_URI'])){
          switch($route['type']){
            case "view":
              $route['filename'] = preg_replace($route['route'], $route['filename'], $_SERVER['REQUEST_URI']);
          }
          preg_match($route['route'], $_SERVER['REQUEST_URI'], $_GET);
          return $route;
        } else {
          continue;
        }
  ...
  /**
   * Universal file extension to mime-type pairings.
   *
   * @var
   */
